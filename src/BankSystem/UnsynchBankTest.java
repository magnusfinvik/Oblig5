...
class Bank {
    ...
    final Condition outOfMoney = lock.newCondition();
    ...
    public void transfer(int from, int to, int amount) throws InterruptedException {
        ...
        try {
            if(accounts[from] > amount) {
                ...
                System.out.println("Account:" + from + " has transferred "
                        + amount + " to " + to);
                accounts[to] += amount;
                ntransacts++;
                outOfMoney.signalAll();
                if (ntransacts % NTEST == 0) test();
            }else{
                System.out.println("Account:" + from + " is out of money");
                outOfMoney.await();
            }
